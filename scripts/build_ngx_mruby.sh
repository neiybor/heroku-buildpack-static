...
# https://github.com/matsumotory/ngx_mruby/blob/v2.6.0/build.sh#L23-L40
...
# The build command fails while compiling a dependency when using multiple threads
NUM_THREADS=1

NGINX_CONFIG_OPT_ENV="${BUILD_OPTS}" NUM_THREADS_ENV="${NUM_THREADS}" ./build.sh
make install
